...
export function DescriptionManage({ show, id, onClose, description }: Props) {
    ...
    const submitHandler = async (e: FormEvent) => {
        e.preventDefault();
        const res: any = await sendRequest("/recipe/edit-description", "PATCH", { description: descriptionInput, id }, {
            "Content-Type": "application/json",
        });
        if (!res.isSuccess) {
            return setError("Adding description to the recipe failed.");
        } else {
            dispatch(editDescriptionRecipeAction({ description: descriptionInput, id }));
            onClose();
        }
    };
                ...
                <form onSubmit={submitHandler}>
                    <VStack spacing={4}>
                        ...
                        <Button type="submit" colorScheme="gray"
                                color="var(--dark)">{description ? "Update" : "Add description"}</Button>
                    </VStack>
                </form>
            }
        </>
