...
type Action =
  ...
  | {
      type: 'SET_DATA';
      inputs: Product | List | ItemInList;
      formIsValid: boolean;
    };
...
export const useForm = (
  initialInputs: Product | List | ItemInList,
  InitialFormValidate: boolean
) => {
  ...
  const selectHandler = useCallback(
    (id: string, value: number, isValid: boolean) => {
      dispatch({
  ...
  const setFormData = useCallback(
    (inputData: Product | List | ItemInList, formValidity: boolean) => {
      dispatch({
        type: 'SET_DATA',
        inputs: inputData,
