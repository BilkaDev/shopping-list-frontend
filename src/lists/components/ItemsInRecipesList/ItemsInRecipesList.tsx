...
export function ItemsInRecipesList({ recipes, category, listId }: Props) {
    ...
    async function deleteItemHandler(recipeId: string, listId: string) {
        const res = await sendRequest(`/list/delete-recipe/${listId}/${recipeId}`, "DELETE");
        if (!res.isSuccess) {
    ...
    const list = recipes.map((recipe) => (
        <div key={recipe.id}>
            <Stack direction="row" paddingTop="1.5rem" justifyContent="space-between" align="center">
                <Text fontSize="4xl">Recipe {recipe.name}</Text>
                <button onClick={() => deleteItemHandler(recipe.id, listId)}><DeleteIcon/></button>
            </Stack>
            <div>
